...
- every project can have its own schema, model settings, and history
...
|  ├── .settings.json    per-project model settings     |
...
- **Per-project settings**: schema in `CLAUDE.md`, model selection in `.settings.json`, project-specific logs and reports
...
Run ingest. The LLM workflow reads the raw source and updates `wiki/` with:
...
| `MEMEX_AI_PROVIDER` | `cli` or `openai_compatible` | `cli` |
| `MEMEX_OPENAI_BASE_URL` | OpenAI-compatible API base URL | unset |
| `MEMEX_OPENAI_API_KEY` | OpenAI-compatible API key | unset |
| `MEMEX_OPENAI_MODEL` | OpenAI-compatible model | unset |
...
The dashboard also persists runtime settings in `.dashboard-settings.json`, and each project can override model selection in its own `.settings.json`.
