...
| Date | Source | Summary |
|------|--------|---------|
| 2026-05-10 | 功能开发 | **Analysis App 自动化生成 (Phase 4/5/6) 完整实现**：10 阶段 Pipeline (Orchestrator) + REST API (6 端点) + MCP 工具 (6 个) + 前端 5 步 Wizard + Hot Questions DB 持久化；新增 AppGenerationTask/Stage/HotQuestion ORM 模型；测试 139 个全部通过。Git 8 commits，涉及 115+ 文件 [^src-2026-05-10-analysis-app-自动化生成-phase456开发记录] |
| 2026-05-09 | 重构 | **LLM 调用统一收敛至 ScopeConfig → LLMConfig**：消除 4 个独立 LLM 配置系统（utils/llms/llm.py、report agent 4 份重复 dataclass、get_peers_utils 裸调用、静默降级）；修复 3 个断掉的调用者；新增 env var 覆盖支持（`{MODEL_NAME}__base_url` / `{MODEL_NAME}__api_key`）；10+ 文件变更 [^src-llm-统一调用收敛至-central-config] |
