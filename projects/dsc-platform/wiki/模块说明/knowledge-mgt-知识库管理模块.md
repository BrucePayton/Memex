...
- **句子分段**：基于标点符号、句子边界进行分段
- **段落分段**：基于换行符和段落结构进行分段
- **递归分段**：基于多个分隔符进行层次化分段
- **Token分段**：基于Token数量进行分段，适配大模型上下文窗口
- **语义分段**：基于语义相似度进行智能分段
- **Markdown分段**：基于Markdown标题结构进行结构化分段
...
- **全文检索**：基于关键词的精确匹配检索
- **语义检索**：基于向量相似度的语义检索
- **混合检索**：结合全文检索和语义检索的结果，综合评分
- **过滤检索**：支持基于元数据的条件过滤检索

#### 检索优化
- 结果重排序：通过交叉编码器或自定义规则对检索结果进行二次排序
- 相似度阈值：可配置最小相似度阈值，过滤低相关结果
- 上下文扩展：自动获取检索结果的相邻上下文块，提供更完整的信息
- 多数据集检索：支持跨多个知识库的联合检索
...
### 2. 智能文档处理
- **OCR识别**：集成PaddleOCR，支持中英文、表格、印章识别
...
### 自定义检索后处理
```python
from knowledge_mgt.knowledge_search.base_reranker import BaseReranker
...
class CustomReranker(BaseReranker):
    ...
    async def rerank(self, query: str, chunks: List[Chunk]) -> List[Chunk]:
        ...
        for chunk in chunks:
            # 计算自定义得分
            chunk.custom_score = self._calculate_score(query, chunk)
...
2. **合理分段**：根据文档类型和使用场景选择合适的分段策略，建议分段大小在500-2000字符，重叠10%-20%
3. **元数据完善**：尽量补充完整的元数据，方便后续的过滤检索
4. **定期维护**：定期清理无效文档，更新过时内容，保持知识库的时效性

### 检索效果优化
1. **选择合适的嵌入模型**：中文场景优先选择中文训练的模型，如BGE-zh、M3E等
2. **混合检索策略**：结合全文检索和语义检索的优势，alpha值建议设置在0.5-0.7
3. **结果重排序**：对检索结果要求高的场景，建议启用交叉编码器重排序
4. **效果评估**：定期对检索效果进行评估，持续优化检索策略

## 常见问题

### Q: 支持多大的文档上传？
A: 默认支持最大100MB的单个文件，可通过`MAX_FILE_SIZE`配置项调整，最大不建议超过500MB。
